...
onde $\text{valor\_producao\_cana}_{it}$ representa o valor total da produção de cana-de-açúcar (em R\$) na microrregião $i$ no ano $t$. Complementarmente, analisamos também a área plantada:
...
\subsection{Filtro Crop-Specific e Tratamento de Zeros Estruturais}

Uma inovação metodológica crucial deste estudo é a aplicação de filtros crop-specific para lidar com zeros estruturais na análise. O procedimento consiste em:

\begin{enumerate}
\item \textbf{Identificação de produtores}: Para cada cultura analisada (cana, soja, arroz), identificamos microrregiões que produziram a cultura em pelo menos um ano durante o período 2003-2021.

\item \textbf{Filtragem da amostra}: A análise principal restringe-se às 225 microrregiões produtoras de cana (51,7\% do total), excluindo 210 microrregiões onde a cana nunca foi cultivada.

\item \textbf{Preservação do painel}: Mantemos todas as observações ano-microrregião para as unidades qualificadas, incluindo anos com produção zero (zeros econômicos, não estruturais).
\end{enumerate}

Esta abordagem oferece vantagens metodológicas importantes:

\begin{itemize}
\item \textbf{Validade do contrafactual}: Garante que comparamos apenas unidades onde o outcome é economicamente viável, evitando comparações espúrias entre regiões produtoras e não-produtoras.

\item \textbf{Preservação de margens}: Mantém tanto a margem extensiva (adoção da cultura) quanto intensiva (expansão/intensificação), pois zeros temporários são preservados.

\item \textbf{Redução de diluição}: Evita que zeros estruturais (impossibilidade agroclimática) diluam efeitos reais observados em regiões produtoras.

\item \textbf{Consistência entre culturas}: O mesmo procedimento é aplicado para análises de soja e arroz, garantindo comparabilidade.
\end{itemize}

A validação empírica confirma a importância desta abordagem: análises sem filtro crop-specific mostram efeitos substancialmente menores e muitas vezes não-significativos, demonstrando como zeros estruturais podem mascarar impactos reais quando incluídos inadequadamente.
...
\begin{figure}[H]
\centering
\caption{Tendências Paralelas - Valor de Produção de Cana-de-Açúcar Normalizado (2003-2021)}
\label{fig:parallel_trends}
\includegraphics[width=0.75\textwidth]{../../../data/outputs/parallel_trends_complete_valor_cana_normalized.png}

Nota: A figura mostra a evolução do valor de produção de cana-de-açúcar médio (em log) separado por grupos de timing de adoção: Early Adopters (2003-2007), Mid Adopters (2008-2012) e Late Adopters (2013-2017), normalizado em t=-1. As áreas sombreadas representam intervalos de confiança de 95\%. A área cinza indica o período pós-tratamento. Observa-se que todos os grupos seguem trajetórias paralelas antes do tratamento (t<0), divergindo apenas após a instalação das estações, com impacto mais pronunciado no valor de produção do que seria observado apenas pela expansão de área.
...
\textit{Notas: *** p<0,01, ** p<0,05, * p<0,10. Todos os modelos utilizam o estimador doubly robust com filtros crop-specific. Cada cultura é analisada apenas nas microrregiões onde é efetivamente produzida, garantindo comparações válidas entre unidades com potencial agrícola similar.}

\textit{Fonte: Elaboração própria a partir dos dados do estudo.}

\vspace{0.3cm}

Os resultados revelam padrões importantes sobre a especificidade e os mecanismos do efeito:

\begin{enumerate}
\item \textbf{Efeito concentrado em cana-de-açúcar}: Tanto o valor de produção (\mainattpct) quanto a área plantada (\areacanaattpct) da cana apresentam efeitos positivos e estatisticamente significativos. A magnitude maior no valor de produção comparada à área indica ganhos de produtividade além da expansão territorial.

\item \textbf{Decomposição do impacto econômico}: A diferença entre o efeito no valor (\mainattpct) e na área (\areacanaattpct) sugere que a informação meteorológica opera através de dois canais complementares. Se o efeito no valor excede substancialmente o efeito na área, isso indica que além da expansão territorial (margem extensiva), há ganhos significativos de produtividade (margem intensiva) derivados de melhor timing de irrigação, escolha de variedades e otimização do manejo.

\item \textbf{Ausência de efeitos em culturas alternativas}: Nem soja nem arroz apresentam efeitos significativos em valor ou área, demonstrando que o impacto não é um fenômeno agrícola geral mas específico às características agronômicas e de manejo da cana-de-açúcar.

\item \textbf{Validação da estratégia de filtragem}: A aplicação de filtros crop-specific garante que cada cultura é analisada apenas entre seus produtores efetivos, evitando diluição de efeitos por zeros estruturais e garantindo comparações válidas.

\item \textbf{Interpretação mecanística}: A especificidade à cana-de-açúcar alinha-se com suas características únicas de manejo hídrico, particularmente a irrigação por salvamento que requer timing preciso baseado em informação meteorológica atualizada.
\end{enumerate}
...
Nota: O gráfico apresenta os efeitos estimados (ATT) e intervalos de confiança de 95\% para as variáveis de resultado analisadas. Os efeitos sobre valor de produção e área plantada de cana-de-açúcar são estatisticamente significativos, enquanto culturas alternativas (soja e arroz) não apresentam impactos significativos, demonstrando a especificidade do efeito. A análise utiliza filtros crop-specific, garantindo comparações válidas entre produtores de cada cultura.
...
Estas evidências combinadas estabelecem uma base sólida para a interpretação causal de que as estações meteorológicas geram impactos econômicos substantivos e sustentados no valor de produção da cana-de-açúcar. O efeito opera através de dois canais complementares: expansão na margem extensiva (aumento de área plantada) e ganhos na margem intensiva (maior produtividade por hectare), resultando em um impacto total no valor que excede o observado apenas na área.
...
\begin{itemize}
\item Impacto no valor de produção: O efeito de \mainattpct{} no valor de produção tem implicações econômicas substanciais. Com um valor bruto de produção de cana-de-açúcar de aproximadamente R\$ 105 bilhões em 2024, este aumento representa potencial de criação de valor significativo para o setor. Importante, este efeito total decompõe-se em expansão de área (\areacanaattpct) e ganhos de produtividade, indicando que a informação meteorológica melhora tanto decisões de alocação quanto de manejo.
...
Este mecanismo explica por que observamos efeitos robustos tanto no \textit{valor de produção} quanto na \textit{área plantada} da cana mas não em culturas alternativas. O impacto maior no valor comparado à área (\mainattpct{} vs \areacanaattpct) indica que a informação meteorológica não apenas viabiliza expansão territorial mas também melhora a produtividade através de decisões otimizadas de manejo.
...
1. Magnitude e Significância do Efeito: O ATT estimado de \mainattpct{} no valor de produção representa impacto econômico substancial. Este efeito é altamente significativo estatisticamente (p < 0,01) e decompõe-se em expansão de área (\areacanaattpct) e ganhos de produtividade, refletindo ajustamentos tanto na margem extensiva quanto intensiva em resposta a melhor informação meteorológica.
...
\item Especificidade ao desfecho: efeito exclusivo ao valor e área de cana-de-açúcar, sem efeitos em culturas alternativas (soja, arroz)
